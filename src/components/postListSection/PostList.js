...
export default function PostList({ $target, initialState, onAdd, onRemove }) {
  ...
  } ${depth === 1 ? "root" : ""}" style="padding-left: ${12 * depth}px">
  ...
  $list.addEventListener("click", async (e) => {
    ...
    if ($li) {
      ...
      if (e.target.classList.contains("add")) {
        ...
        const currentOpened =
          JSON.parse(localStorage.getItem(CLOSED_POSTS_ID_STORAGE_KEY)) || [];

        if (isClosed) {
          currentOpened.push(id);
          localStorage.setItem(
            CLOSED_POSTS_ID_STORAGE_KEY,
            JSON.stringify(currentOpened)
          );
        } else {
